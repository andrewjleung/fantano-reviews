...
# After the review for Flying Lotus' Cosmogramma, review videos started
# being formatted with new review types.
...
# This matches against both old and new title formatting but doesn't catch
# some edge cases.
...
# Labels that erroneously get recognized as genres since some reviews
# aren't tagged with genres.
...
def get_genres(description):
    ''' Function:   _get_genres
        Parameters: description, the review video's description
        Returns:    string, the genres within the given description,
                        delimited by semicolons and ready to be placed
                        in a CSV, None if there's no genres
    '''
    def is_genre(genre):
